...
# generic
	* class extends Visitor [HTMLElement]
	* 순회제어코드 

# visitor pattern
	* visotor pattenr을 사용해서 제어의역전이 일어나서 효율서도 visitor pattern이 적용된 곳에서 
	* 어려운 자료구조나 특별한 코드에대해서 제어코드가 완전히 외부로 위임 되서 한군데 몰려 있다. 
	* 결과
		- Scanner가 Dom제어 코드를 갖고 있지 않고 
		제어를 외부에 위임 하기 위해서코드를 객체려 바꿈
		(이전 처럼 역할때문에 코드를 객체로 바꾼건 아니다)
		
	* action(curr)visitor를 호출한 부분과 함께 하는 제어 부분 
		- action: function
		- target: el
		
		- 제어를 관여 할 부분 
		- 마치 템플릿 메소드 훅과 같은 역할 
		- 그래서 visitor pattern은 care taker와 긴밀한 역할을 합니다. 
		- 제어는 Visitor가 가져가지만 care taker는 특정 메소드와 대화 하게 된다. 
		
# Scanner
	* constructor에 Domvisitor객체를 넘겨준다.  
	* dom loop가 사라졌다. visitor pattern으로 이동 
	왜? Scanner 역할이 아니라고 판단 됐기  때문에
	다른 Dom loop가 필요할 곳에서도 사용 될 수 있기 때문이다. 
	
# architecture
	* 코드의 재배치 
	* 객체지향에서는 코드 역할 때문에 재배치한다. 
